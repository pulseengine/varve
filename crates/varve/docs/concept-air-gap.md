...
**One archive carries one platform.** `archive` exports what the archiving machine installed, and `install` fetches only its own platform's payloads, so an archive of a four-platform layer holds the payloads for the archiving host and nothing for the other three. `archive` prints the count it carried and the entries it omitted. A mixed site needs one archive per platform — but **not one machine per platform**. `varve install --platform <triple>` lays a core down for another target from a source that carries it, and `varve archive --platform <triple>` then exports that core. One connected machine can therefore prepare every platform's archive offline, for a site with none. The bytes are on the machine whenever the source carried them; only a *host-platform* install leaves the other platforms behind. Installing an archive on a platform it does not carry is refused before anything lands, naming both triples.
